...
def adamGD(batch, num_classes, lr, dim, n_c, beta1, beta2, params, cost):
    ...
    X = X.reshape(len(batch), n_c, 20, 44)
...
def train(num_classes = 8, lr = 0.01, beta1 = 0.95, beta2 = 0.99, img_dim = 28, img_depth = 1, f = 3, num_filt1 = 8, num_filt2 = 8, batch_size = 32, num_epochs = 2, save_path = 'params.pkl'):

    # Get training data
    data = pd.read_pickle('dataset.pkl')
    train, test = train_test_split(data, test_size=0.2, random_state=42, shuffle=True) # training 80%
    train = train.reset_index().drop('index', axis=1)
    #print(train.head(), len(train), sep='\n')

    ord_enc = OrdinalEncoder()
    train['accent_code'] = ord_enc.fit_transform(train[['accent']]).astype(int)
    #print(train[['accent', 'accent_code']].head(11))

    train = train.drop('sex filename accent'.split(), axis=1)
    y = []
    X = []
    ...
    f1, f2, w3, w4 = (num_filt1,img_depth,f,f), (num_filt2,num_filt1,f,f), (128,512), (8,128)
